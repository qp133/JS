...
function BillInformation({onList}) {
  const total = onList.map((item) => {
    return item.price * item.count;
  })

  const subTotal = total.reduce((a,b) => a + b)
    ...
    <div className="col-md-4">
        <div className="bill">
          ...
          <div className="border mb-2 p-3 fs-5 fw-normal d-flex justify-content-between align-items-center">
            ...
            <span className="text-primary" id="total-money">{subTotal + subTotal/10} VND</span>
          </div>
        </div>            
    </div>
  )
